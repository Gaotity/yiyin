...
Verified against current `main` (the post-rewrite Tauri/Rust/React tree). All live-relevant findings handled; the Svelte-migration item has no analog.

- `any` escapes on the EXIF chain → ALREADY-HANDLED — no TypeScript EXIF chain exists; metadata normalization is Rust-owned; `strict` + `noUncheckedIndexedAccess` on; zero `: any`/`as any`/`@ts-ignore` in `src/` and `tests/`
- zod ↔ TS drift (`FontSettingsOverride`, `font.use`) → ALREADY-HANDLED — no zod dependency; single Rust `FontSpec` model with validation (`crates/yiyin-domain/src/template.rs:38-53`); legacy `use` accepted via serde rename at the boundary (`json_repository.rs:592-648`); DTO generated from Rust with a sync test (`src-tauri/tests/dto.rs`)
- `logger/` lint exclusion → ALREADY-HANDLED — eslint is gone; Biome's only exclusion is the generated `src/platform/types.ts` (`biome.json:6`)
- Svelte 5 migration / svelte-check errors / `pnpm ci` gate → LEGACY-ONLY — no svelte dependency; the migration premise is void
