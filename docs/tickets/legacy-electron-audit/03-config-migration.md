...
Verified against current `main` (the post-rewrite Tauri/Rust/React tree). All seven findings are already handled in the rewrite, with tests.
...
- single safeParse resets all blocks on one bad field → ALREADY-HANDLED — serde per-struct `deny_unknown_fields` plus domain validation (`json_repository.rs:300-312,457-485`); legacy import fails atomically and never mutates the source (`legacy_import.rs:82-87`)
