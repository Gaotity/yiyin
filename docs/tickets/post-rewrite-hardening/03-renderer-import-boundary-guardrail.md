...
**Acceptance criteria:**
- [ ] The policy test scans `src/` and fails on `@tauri-apps/api` or `node:*`
      imports outside `src/platform/`
- [ ] The test is proven red against a planted violation (fixture or temporary
      file), then green on the real tree
- [ ] Full frontend suite (`pnpm test`) and policy suite stay green
